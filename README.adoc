...
Before the service reports its state, it should check what it has. Depending on what the service
has or provided to the it, it can report whether it is healthy or not via the Microprofile Health
API implementation. A self check can be anything related to the service. It could be a dependency,
a successful connection to an endpoint, a system property and many others.
...
If the service displays the system properties as desired, then the health of this service is `UP`.
...
* The `testIfServicesAreUp()` method sends a request to `/health` endpoint then it verifies the
`200` response code is returned. After that, it calls the `connectToHealthEndpoint()` helper method
