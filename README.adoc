...
:page-seo-title: Checking the health of Java microservices by using MicroProfile Health Check
:page-seo-description: A getting started tutorial and an example on how to monitor and report the health status of Java microservices by implementing readiness and liveness checks using MicroProfile Health.
...
// static guide instructions:
ifndef::cloud-hosted[]
While the server is running, navigate to the http://localhost:9080/health[http://localhost:9080/health^] URL to find the aggregated startup, liveness, and readiness health reports on the two services.
