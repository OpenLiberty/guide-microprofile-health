...
1. Find the provided property of type boolean, shown in the following code, in the `resources/CustomConfigSource.json` file. The InventoryConfig object reads the property in this file, which manipulates the state of the service.
```
...
Now, turn the `inventory` service down to simulate that it is in maintenance or because something went wrong. Change the
...
Use the `static` keyword with curly brackets to initialize the static fields, such as the
...
* The `testIfServicesAreUp()` method sends a request to `http://localhost:9080/health`. Next, it
...
request to check the services' states again. This time, the expected response code is `503`.
Lastly, the method asserts if the state of the `inventory` service is `DOWN` and the state of 
...
Next, you can try one of the related MicroProfile guides, which you can use to expand on what you just built.
