...
you will report the `inventory` service as not ready if the service is in maintenance or if its dependant service is unavailable.

For simplicity and simulation, the custom [hotspot=3 file=2]`io_openliberty_guides_inventory_inMaintenance`
...
Moreover, the readiness health check procedure makes an HTTP [hotspot=getRequest]`GET` request to the `system` service and check its status.
If the request is successful, the `inventory` service is healthy and ready since its dependant service is available.
...
you want learn more about MicroProfile Config, see
