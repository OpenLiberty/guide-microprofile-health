...
Learn how to use a Microprofile health check on a RESTful service to report its state in an
application.
...
in the application is `UP` if all services are up. When they happen to be all up, the health endpoint
responds with `200` code meaning a positive outcome. Otherwise, if any service is down, the
outcome is `DOWN` and the health endpoint will respond with `503` code meaning outcome is negative.

You will add a health check to report the states of `InventoryResource` and `PropertiesResource`
RESTful services. These two services are provided in
https://github.com/OpenLiberty/draft-guide-rest-cdi[Creating a RESTful web service with contexts and dependency injection].
Then you will write tests to validate the states of these services.
...
method from this interface to make it simulate the response (i.e. the state) to be up
if the system property `wlp.server.name` is the `defaultServer`. Otherwise, set the state to be
down. In addition, pass the service name which is `PropertiesResource` as an argument to `named()`
method to know which service is being reported.
...
1. A property of type boolean found in `/start/CustomConfigSource.json`:
    ...
    "io.openliberty.guides.microprofile.inventory.inMaintenance": false
      ...
      This property in the file is provided here to be used in this guide.
      ...
      creating an `InventoryConfig` object which will do that for you.
...
The `@Inject` annotation is a dependency injection. You can use it
to initialize an `InventoryConfig` object. This object will allow you to read
that property in `CustomConfigSource.json` file. You can read more about `@Inject` from
https://github.com/OpenLiberty/draft-guide-rest-cdi[Creating a RESTful web service with contexts and dependency injection].

The `isHealthy()` method returns a boolean that will decide the state of
`InventoryResource` in `call()` method. It does two things. First, it gets that property's value using
`InventoryConfig` object then it checks if `PropertiesResource` is available or performing by
checking if the response is `200`. This method only returns true when that property's value is
`false` and `PropertiesResource` is available (i.e. /system/properties is performing).

The `isHealthy()` method is used as the condition to determine the state of `InventoryResource` in
`call()` method. The state of `InventoryResource` service is `UP` if `isHealthy` returns `true` and
vice versa.
...
As you can see, the service is now handling itself. Specifically, `getPropertiesForHost()` and
`listContents()` methods were adjusted to work corresponding to that property's value. Any change
to that property 's value will be read by the `InventoryConfig` object in these methods
and in `call()` method in `InventoryHealth`.

That `call()` method its job is to update the state of `InventoryResource` service state on `/health`
endpoint. For the other two methods, their job is to either make the `InventoryResource` service
function normally (i.e. available) or indicate that the service is currently on maintenance.

To make it clear, the service is only in maintenance when its state is `DOWN`
(i.e. value of that property is `false`). Also, the service is only available when its state is
`UP` (i.e. value of that property is `true`).
...
The fields shown are all used to perform the tests. They are the `JsonArray` that hold the states
of the services, the two maps for storing the states when the services are up and when they are
down and the constant to hold the property that will be read.

The `setup()` method initializes the data needed for the two tests. The `@Before` annotation
is needed to call that method before any other methods.

The `testIfServicesStatesUp()` test method with `@Test` annotation checks if both services are
currently `UP` using the helper method `connectToHealthEnpoint()`. This helper method sends a request
to `\http://localhost:9080/health` then checks the `200` response code and reads the
response as `JsonArray` which contains the states of the two services. The `checkServicesStates()`
method checks and asserts the actual states of both services using the helper method
`getActualState()`. This helper method returns the state (`UP` or `DOWN`) of a specific service
from the `JsonArray` object that is passed as an argument to it.

The `testIfInventoryTemporarilyDown()` method follows the same procedure as in
`testIfServicesStatesUp()` method. However, It sends another request after changing the
property's value, `inMaintenance`, from `false` to `true` . Then it validates the state of the
service `InventoryResource` by asserting if it is `DOWN` and the other service is `UP`.

The `teardown()` method annotated it with `@After` calls the helper method `cleanUp()`
to reset the property's value after the tests are done.
