...
:page-seo-title: Checking the health of microservices using MicroProfile Health Check
:page-seo-description: A tutorial on how to report the health status of Java microservices by implementing readiness and liveness checks using MicroProfile Health Check.
...
have been provided for you, and implement what is necessary to report health status by 
...
show the state of the `inventory` service. As you might expect, both services are in the 
`UP` state, and the overall health status of the application is `UP`.
...
URL to view the data from the readiness health checks. Similarily, access the `/health/live`
...
A readiness check allows third-party services, such as Kubernetes, to know if the microservice 
is ready to process requests. For example, checking dependencies such as a database connection. 
A liveness check allows third-party services to determine if the microservice is running. 
If the liveness check fails, this means that the application can be terminated. For example, 
...
----
include::finish/src/main/java/io/openliberty/guides/system/SystemReadinessCheck.java[tags=SystemReadinessCheck]
----

The [hotspot=8 file=0]`@Readiness` annotation indicates that this is a readiness health check procedure. 
By pairing this annotation with the [hotspot=9]`ApplicationScoped` context from the Contexts and 
...
[source, java, linenums, role='code_column']
----
include::finish/src/main/java/io/openliberty/guides/system/SystemLivenessCheck.java[tags=SystemLivenessCheck]
----

The [hotspot=11 file=1]`@Liveness` annotation indicates that this is a liveness health check procedure.
In this case, you are checking the heap memory usage. If more than 90% of the maximum memory
is being used, a status of `DOWN` will be returned.  
...
[source, java, linenums, role='code_column']
----
include::finish/src/main/java/io/openliberty/guides/inventory/InventoryReadinessCheck.java[tags=InventoryReadinessCheck]
----
...
Based on these two factors, the [hotspot=19-36 file=0]`isHealthy()` method returns whether 
...
[source, java, linenums, role='code_column']
----
include::finish/src/main/java/io/openliberty/guides/inventory/InventoryLivenessCheck.java[tags=InventoryLivenessCheck]
----

Just like the `system` liveness check, you are checking the heap memory usage. If more 
than 90% of the maximum memory is being used, a status of `DOWN` will be returned.  
...
You can also navigate to http://localhost:9080/health/ready[http://localhost:9080/health/ready^]
...
You will implement several test methods, `testIfServicesAreUp()`, `testReadiness`, `testLiveness` and 
`testIfInventoryServiceIsDown()`, to validate the health of the `system` and `inventory` services.
...
- The [hotspot=25-33 file=0]`testIfServicesAreUp()` test case compares the generated health report 
with the actual status of the services. 
- The [hotspot=36-42 file=0]`testReadiness()` test case compares the generated health report for the 
readiness checks with the actual status of the services. 
- The [hotspot=45-51 file=0]`testLiveness()` test case compares the generated health report for the 
liveness checks with the actual status of the services. 
- The [hotspot=54-68 file=0]`testIfInventoryServiceIsDown()` test case puts the `inventory` service 
