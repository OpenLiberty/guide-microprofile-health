...
the `isInMaintenance()` called by the `InventoryConfig` object returns `false`. This happens when you set
...
response code. Thus, ff the service is down for maintenance, its health is `DOWN`.
