...
MicroProfile Health allows services to report their health and it publishes overall health status to a defined
endpoint.  A service reports `UP` if it is available and reports `DOWN` if it is unavailable.  MicroProfile Health reports
individual service status at the endpoint and indicates the overall status as `UP` if all the services are `UP`.  A service
orchestrator can then use the health statuses to make decisions.

A service checks its own health by performing necessary self-checks and then report its overall status via
implementing the API provided by MicroProfile Health.  A self-check can be a check on anything that is
needed by the service, such as a dependency, a successful connection to an endpoint, a system property, a database, or
availability of required resources.

You will add self-checks to the `system` and `inventory` services, which have been provided to you, and implement what is
necessary to report health status using MicroProfile Health.
...
This class implements the `HealthCheck` interface from MicroProfile Health API. The class also implements the `call()``
