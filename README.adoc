...
The health check is used to assess the states of the services in an application. The state of a service
is either `UP` or `DOWN`. When the state is `UP`, the service is available. However,
when it is `DOWN`, the service is currently not available (i.e. in maintenance).
The overall outcome of the application is `UP` only when the checks of all the services are `UP`.

Before the service reports its states, it should check what it needs or has. Depending on what is
provided, it can report whether it is healthy or not via the Microprofile Health API implementation.
A self check can be anything related to the service. It could be a dependency, a successful
connection to an endpoint, a system property and many others.
...
applicaiton. As shown, there are two checks. One that shows the state of `system`
...
You can also invoke the two RESTful services by pointing to their endpoints:

- http://localhost:9080/system/properties refers to the `system` service that retrieves
the information for a specific host
- http://localhost:9080/inventory/hosts refers to the `inventory` service that retrieves
the information for a list of all previously registered hosts
...
Proceed with the two sections below to add health checks to `system` and
`inventory` services.
...
Create `PropertiesHealth.java` class in `/start/src/main/java/io/openliberty/guides/system/` to
report the health of the `system` service.
...
to report the health of the `inventory` service.
...
This service reports its healthy state depending on these two self checks:

1. A property of type boolean found in `/start/CustomConfigSource.json`. This property is used this
file to manipulate on the state of the service. This file is provided for you in this guide. If
you want to learn how to configure this file, see https://openliberty.io/guides/microprofile-config-intro.html[Configuring Microservices using MicroProfile Config].
...
`call()` method. The state of `inventory` service is `UP` if `isHealthy` returns `true`
and vice versa.

Now, open `inventory` in `/start/src/main/java/io/openliberty/guides/inventory/` to add a
condition to this service.
...
`getPropertiesForHost()` method. This can only happen if the
...
If the service is down for maintenance, then its health is `DOWN'. Notice the second condition in
the `isHealthy()` method of the `InventoryHealth` class is relying on this setting.
...
that the service is currently down for maintenance. You just need to pass the name of the service
to it.
...
verify that by pointing your browser to the inventory service endpoint. As you can see, the service
...
The `static` keyword with curly brackets is used to initializes the static fields. They are the
`JsonArray` that hold the states of the services, the two maps for storing the states when the
services are up and when they are down and the constant to hold the property that will be read.

The `@After` annotations is placed on methods that execute after every test case. These methods are
...
* The `testIfServicesAreUp()` method sends a request to `\http://localhost:9080/health` then
verifies the `200` response code is returned. After that, it reads the response as `JsonArray` which
contains the states of the two services. It does that using `connectToHealthEndpoint()` helper
method. Then, this `JsonArray` is passed to `checkServicesStates()` method to compare and assert
it with the expected services states. It does that using `getActualState()` helper method which
returns the state (`UP` or `DOWN`) of a specific service from the `JsonArray` object that is
passed as an argument. The expected state for both of the services in this test is `UP`.
...
Lastly, we validate the state of the `inventory` service by asserting if it is `DOWN`
but the `ProeprtiesResource` service is `UP`.
