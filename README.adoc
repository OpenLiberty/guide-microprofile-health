...
is ready to process requests. For example, verifying a database connection. A liveness check 
allows third-party services to determine if the microservice is running. If the liveness check
fails, this means that the application can be terminated. For example, if the application 
runs out of memory. 
