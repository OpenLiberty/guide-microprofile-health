...
:page-seo-title: Checking the health of microservices by using MicroProfile Health Check
:page-seo-description: A tutorial on how to report the health status of Java microservices by implementing readiness and liveness checks by using MicroProfile Health Check.
...
To try out the services, first go to the `finish` directory and run the
...
After you are done checking out the services, stop the Open Liberty server by pressing `CTRL+C` in the shell session where you ran the server.
...
The [hotspot=8 file=0]`@Readiness` annotation indicates that this particular bean is a readiness health check procedure.
...
[source, java, linenums, role='code_column']
----
include::finish/src/test/java/it/io/openliberty/guides/health/HealthIT.java[tags=HealthTest]
----
...
[source, Java, linenums, role='code_column']
----
include::finish/src/test/java/it/io/openliberty/guides/inventory/InventoryEndpointIT.java[tags=**;!copyright]
----

SystemEndpointIT.java
[source, Java, linenums, role='code_column']
----
include::finish/src/test/java/it/io/openliberty/guides/system/SystemEndpointIT.java[tags=!copyright]
----
...
[INFO] Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.102 s - in it.io.openliberty.guides.inventory.InventoryEndpointIT
[INFO]
[INFO] Results:
[INFO]
...
You will see a test failure occur because the initial status of the `inventory` service is `DOWN`.
