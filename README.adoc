...
:page-description: Learn how to provide and check the health of microservices by using MicroProfile Health.
...
All applications must provide an implementation of the `HealthCheck` interface, which will be used to
verify the healthiness of the services.
...
By pairing this annotation with the `ApplicationScoped` context from the CDI API, the bean is discovered
...
Overal, this is a very simple implementation of the `call()` method. In a real development environment,
...
To check that the service is down, simply make a HTTP GET request to itself and check the status
returned by the response. If the status is not 200, then the service is not running. Based on these
