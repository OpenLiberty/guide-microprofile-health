...
method. For a real environment, you would want to orchestrate much more meaningful
health checks.
...
This readiness check will check to see if the `inventory` service is in maintenance.
The [hotspot=isHealthy file=0]`isHealthy()` method returns the readiness status of the `inventory`
service. A custom [hotspot=3 file=2]`io_openliberty_guides_inventory_inMaintenance`
MicroProfile Config property has been defined for you in the [hotspot file=2]`resources/CustomConfigSource.json` file, which indicates whether the `inventory` service is in maintenance or not. Since the `inventory` service in your application depends on the `system` service, it will be unhealthy if the `system` service is down. For this reason, the status of the `system` service is also checked.

To check whether the service is up, make an HTTP GET request to the `system` service and check the status that is returned by the response. If the status code is not `200`, then the `system` service is not running. Upon receiving a successful response, the [hotspot=isHealthy file=0]`isHealthy()` method returns a healthy `UP` state for the `inventory` service since the MicroProfile Config maintenance property is defined as [hotspot=3 file=2]`false` and is defined in your readiness check as an [hotspot=inventoryConfig file=0]`inventoryConfig` object.

To learn more about MicroProfile Config and its properties, see https://openliberty.io/guides/microprofile-config.html[Configuring microservices^].
