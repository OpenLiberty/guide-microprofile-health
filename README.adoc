...
is currently not available (i.e. on maintenance). The overall outcome of the application is `UP` only when the checks of all the services are `UP`. 
...
You will add health checks to report the states of `PropertiesResource` and `InventoryResource`
...
You can also verify states of the two RESTful services that by pointing to their endpoints:
...
you to read the property in `CustomConfigSource.json` file. You can learn more about `@Inject` from
...
It does the two things that were mentions above. This method is used as the condition in `call()` method. The state of `InventoryResource` service is `UP` if `isHealthy` returns `true` and vice versa.
...
`getPropertiesForHost()` method. This can only happen if the `io.openliberty.guides.microprofile.inventory.inMaintenance` configuration is set to `false`. Otherwise the service responds with the message, `Service is temporarily down for maintenance`. The configuration condition check `config.isInMaintenance()` is the key here. The `isHealthy()` method in `InventoryHealth` class is also using this condition to report the state of `InventoryResource` service. If the service reports the system properties as desired, then the health of this service is `UP`. If the service is down for maintenance, then its health is `DOWN'.

This is the same simulation is implemented for the for `listContents()` method which lists the stored hosts.
