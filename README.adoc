...
To check whether the service is up, make an HTTP GET request to the `system` service and check the status that is returned by the response. If the status code is not `200`, then the `system` service is not running. Upon recieving a successful response, the [hotspot=isHealthy file=0]`isHealthy()` method returns a healthy `UP` state for the `inventory` service since the MicroProfile Config maintenance property is defined as [hotspot=3 file=2]`false` and is defined in your readiness check as an [hotspot=inventoryConfig file=0]`inventoryConfig` object.

To learn more about MicroProfile Config and it's properties, see https://openliberty.io/guides/microprofile-config.html[Configuring microservices^].
