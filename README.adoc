...
The `@Health` annotation indicates that this class will be used to report the state of
`system` service to the `http://localhost:9080/health` URL. The `@ApplicationScoped` annotation is
required because the health check service must stay active within the lifetime of the application
to report the state.

This class implements the `HealthCheck` interface from MicroProfile Health API. The `call()` method with `@Override`
annotation from the interface is used to report the state of the service by returning a `HealthCheckResponse` object.
...
`io.openliberty.guides.microprofile.inventory.inMaintenance` configuration to `false`. If this is the case,
then the health of this service is `UP`.

Otherwise, if the configuration is set to `true`, the service responds with a `ERROR: Service is currently in maintenance. Please contact: admin@guides.openliberty.io`
