...
its health such as in a cloud or containerized environment.

MicroProfile Health enables services to report their health, and it publishes the overall health status to a defined
endpoint.  A service reports `UP` if it is available and reports `DOWN` if it is unavailable.  MicroProfile Health reports individual service status at the endpoint and indicates the overall status as `UP` if all the services are `UP`.  A service
...
You can access the `inventory` and `system` services to test their availability. Point your browser at the
`http://localhost:9080/inventory/systems` URL to access the `inventory` service and at the
 `http://localhost:9080/system/properties` URL to access the `system` service.
...
The `@Inject` annotation is used to initialize the `InventoryConfig` object. Use this object to read the property
in the `resources/CustomConfigSource.json` file.
...
The `getPropertiesForHost()` method displays the system properties for a host name only if the `isInMaintenance()` method
that is called by the `InventoryConfig` object returns `false`. The method returns `false` when you set the
`io.openliberty.guides.microprofile.inventory.inMaintenance` property to `false`. Since the `InventoryHealth` class also
reads this property through the `isHealthy()` method, the health of this service is `UP` because it is not in maintenance.
Otherwise, if the property is set to `true`, the service responds with an error message and a `503` response code. Thus,
if the service is down for maintenance, its health is `DOWN`.
