...
The warning messages are expected. The first warning results from [hotspot=testIfInventoryServiceIsDown file=0]`testIfInventoryServiceIsDown()` making a request to a service that is down due to being in maintenance. The second warning and error results from a request to a bad or an unknown host name. This request is made in the test [hotspot=testUnknownHost file=4]`testUnknownHost()` from [hotspot file=4]`InventoryEndpointIT`.
