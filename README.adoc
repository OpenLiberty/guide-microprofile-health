...
Navigate to the `pom.xml` file to check the required dependency. The `microprofile-health-api` dependency is added so you
can use the MicroProfile Health API to provide health checks for your microservices.
The `mpHealth-1.0` feature is also enabled in the `src/main/liberty/config/server.xml` file.
