...
As you can see, there are two checks. One shows the state of the `system` service, and the other shows the state of
the `inventory` service. As you might expect, both services are in the `UP` state, and the overall health status of
the application is `UP`. In each check, the data field is left empty for simplicity. This field can be
filled with custom information regarding the health of the two services.
