...
Point to `http://localhost:9080/health` URL. From here, you can see the health of the two services as well as the overall
...
a service to the `http://localhost:9080/health` URL. The `@ApplicationScoped` annotation is
...
To verify the states, point to the
`http://localhost:9080/inventory/systems` URL of `inventory` service. The service
