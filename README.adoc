...
returning [hotspot=HealthCheckResponse-UP]`UP` if it is, and [hotspot=HealthCheckResponse-DOWN]`DOWN` otherwise. The 
[hotspot=HealthCheckResponse-named]`HealthCheckResponse.named()` method is used to indicate what service the health 
check is done for. Overall, this is a very simple implementation of the [hotspot=healthCheckResponse]`call()` 
...
HealthIT.java
[source, java, linenums, role='code_column hide_tags=copyright tags=HealthTest']
----
