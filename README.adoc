...
You’ll explore how to check the health of a microservice with MicroProfile Health.
...
`http://localhost:9080/inventory/systems/localhost` microservice retrieves the information for a specific host. The
`http://localhost:9080/inventory/systems` microservice retrieves the information for a list of all previously
registered hosts.
...
service, and the other shows the state of the `inventory` service. As you might expect,
both services are in the `UP` state, and the overall outcome of the application is `UP`.
...
Create the `src/main/java/io/openliberty/guides/system/SystemHealth.java` class:
...
Create the `src/main/java/io/openliberty/guides/inventory/InventoryHealth.java` class:
...
To learn how to configure the file, see https://openliberty.io/guides/microprofile-config.html[Configuring Microservices using MicroProfile Config].
...
https://openliberty.io/guides/cdi-intro.html[Handling dependencies with RESTful web services].
...
the system properties for a host name. The host name displays only if you set the
...
Begin by creating a `/src/test/java/it/io/openliberty/guides/health/HealthTest.java` test class:
...
verifies that the `200` response code is returned. Next, it calls the `connectToHealthEndpoint()` helper method
