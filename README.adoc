...
The warning messages are expected. The first results from [hotspot=testIfInventoryServiceIsDown file=0]`testIfInventoryServiceIsDown()` making a request to a service that is down due to being in maintenance. The second warning results from a request to a bad or an unknown host name.
